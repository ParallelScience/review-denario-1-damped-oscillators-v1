...
- Studies a practically important system-identification question: how noise level (SNR) and sampling resolution constrain estimation of $k$ and $b$ from $x$ and $\dot{x}$ measurements (Sec. 1).
...
4.  **SNR is central to the claims but is not mathematically defined, and it is unclear how SNR is enforced when noise is injected into both $x$ and $\dot{x}$ (Sec. 2.1, Sec. 2.3). Different SNR conventions (power vs variance, per-channel vs joint, computed before/after downsampling) can materially change threshold values.**
...
6.  **The DKF (Sec. 2.2.2) is described only qualitatively and is not reproducible or assessable for fairness. Missing are: the discrete-time state transition model for $[x,\dot{x}]$ as a function of $(k, b, \Delta t)$ and whether discretization is exact or approximate; the parameter evolution model (constant vs random walk); the measurement model for observing $x$ and $\dot{x}$; full $Q/R$ covariance choices and tuning; coupling between the dual filters; initialization (means/covariances); handling of parameter constraints ($k > 0$, $b > 0$); and sensitivity to priors/tuning.**
...
1.  The abstract and Introduction (Sec. 1) state specific threshold values but (with Sec. 3 empty) do not point to any supporting figure/table, making these numbers appear ungrounded.
    ...
    *Recommendation:* After adding Sec. 3 results, update the Abstract/Sec. 1 to cite the exact figures/tables (e.g., “Fig. X”) from which thresholds are read, and ensure reported numbers match plotted values (use “$\sim$”/“around” unless precision is justified).
...
2.  The downsampling/noise/smoothing order of operations is unclear (Sec. 2.1, Sec. 2.2.1). It is ambiguous whether noise is added before/after downsampling, whether anti-aliasing filtering is used, and where Savitzky–Golay smoothing sits in the pipeline.
    ...
    *Recommendation:* Add an explicit step-by-step pipeline description (bullet list) in Sec. 2.1/Sec. 2.2.1: simulate at base $\Delta t$ $\rightarrow$ (optional anti-alias) $\rightarrow$ downsample $\rightarrow$ inject noise (or vice versa, but justify) $\rightarrow$ apply smoothing $\rightarrow$ differentiate $\rightarrow$ regress; and mirror the same $\Delta t$ handling in the DKF discretization (Sec. 2.2.2).
...
6.  State vector notation in Sec. 2.2.2 appears inconsistent/typoed (e.g., $z = [x, x, k, \dot{b}]^T$) and another list appears malformed (e.g., $(x, x, \dot{x}, \ddot{x})$).
    
    *Recommendation:* Correct to unambiguous notation throughout (e.g., $z = [x, \dot{x}, k, b]^T$ and $(x, \dot{x}, \ddot{x})$), and ensure dots indicate time derivatives only where intended.

7.  MAPE equation formatting (Eq. (3)) may be ambiguous regarding parentheses.
    
    *Recommendation:* Rewrite Eq. (3) with explicit parentheses: $\text{MAPE} = \frac{1}{N} \sum_i \left| \frac{\theta_i - \hat{\theta}_i}{\theta_i} \right| \times 100\%$.

8.  The Introduction uses a forced equation $m\ddot{x} + b\dot{x} + kx = F(t)$ but the Methods use the unforced case (Eq. (1)) without explicitly stating $F(t)=0$ in simulation.
...
1.  ✔ **Free oscillator governing ODE** (Eq. (1), Sec. 2.1, p.2)
    ...
    - **Claim:** The simulated underdamped oscillator satisfies $m\ddot{x}(t) + b\dot{x}(t) + kx(t) = 0$.
    ...
    - **Notes:** Units are consistent: $m\ddot{x}$ has force units; $b\dot{x}$ requires $b$ to have units of mass/time; $k x$ requires $k$ to have units of mass/time$^2$. The equation matches the described model in Methods.

2.  ✔ **Regression form derived from ODE** (Eq. (2), Sec. 2.2.1, p.3)
    
    - **Claim:** Rearranging Eq. (1) yields $\ddot{x}(t) = -\frac{b}{m}\dot{x}(t) - \frac{k}{m}x(t)$.
    - **Checks:** algebraic derivation, dimensional consistency
    - **Verdict:** PASS; confidence: high; impact: critical
    - **Assumptions/inputs:** $m \neq 0$, Eq. (1) holds
...
4.  ✖ **DKF augmented state vector definition** (Sec. 2.2.2, p.3)
    ...
    - **Claim:** The system is modeled with state vector $z = [x, x, k, \dot{b}]^T$ and DKF estimates kinematic state ($x$, $\dot{x}$) and parameters ($k$, $b$).
    ...
    - **Notes:** The written vector repeats $x$ (second entry should presumably be $\dot{x}$) and includes a dot on $b$ (suggesting $\dot{b}$), contradicting the surrounding text that parameters are ($k$, $b$). This is an internal inconsistency in notation that should be corrected.

5.  ⚠ **MAPE definition and parentheses** (Eq. (3), Sec. 2.3, p.4)
    
    - **Claim:** $\text{MAPE} = \frac{1}{N} \Sigma_i |(\theta_i - \hat{\theta}_i)/\theta_i| \times 100\%$.
    - **Checks:** algebraic/formula clarity, definition consistency
    - **Verdict:** UNCERTAIN; confidence: medium; impact: minor
    - **Assumptions/inputs:** $\theta_i$ is ground-truth parameter for oscillator $i$, $\hat{\theta}_i$ is the corresponding estimate, $\theta_i \neq 0$ for all $i$
    - **Notes:** The intended MAPE is clear from the prose, but the displayed fraction/absolute-value grouping is potentially ambiguous without explicit parentheses. If interpreted as $|\theta_i - (\hat{\theta}_i/\theta_i)|$ it would be nonstandard and dimensionally inconsistent; if interpreted as $|(\theta_i - \hat{\theta}_i)/\theta_i|$ it is dimensionless and consistent.
...
7.  ✔ **Forced vs unforced dynamics consistency** (Introduction (p.2) vs Eq. (1) (p.2))
    ...
    - **Claim:** The paper references $m\ddot{x}+b\dot{x}+kx=F(t)$ in the introduction but simulates $m\ddot{x}+b\dot{x}+kx=0$ in methods.
...
5.  ✔ **C5** (Page 3, Section 2.1)
    ...
    - **Verdict:** PASS
    - **Notes:** Computed $\Delta t = 20/n$ for $n$ in $\{500,250,100,50,25\}$ yielding $\{0.04, 0.08, 0.2, 0.4, 0.8\}$ s; matched expected values and strictly increased as $n$ decreased.

6.  ✔ **C6** (Page 3, Eq. (2) and Page 2, Eq. (1))
    
    - **Claim:** Equation (2) rearranges equation (1) into $\ddot{x}(t) = -\frac{b}{m}\dot{x}(t) - \frac{k}{m}x(t)$.
    - **Checks:** algebraic consistency (symbolic sign/term check)
    - **Verdict:** PASS
    - **Notes:** Structural/algebraic consistency check: dividing $m\ddot{x} + b\dot{x} + kx=0$ by $m$ and solving for $\ddot{x}$ gives negative $(b/m)\dot{x}$ and $(k/m)x$ terms.
